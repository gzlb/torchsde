...
#
# We begin by defining the generator SDE. The idea is to define one large SDE for both, reflected in one generator and one discriminator 
###################
class GeneratorFunc(torch.nn.Module):
    ...
    def __init__(self, noise_size, hidden_size, mlp_size, num_layers):
        ...
        # [-3, 3] rather than [-1, 1].
        ###################
        self._drift_stock = MLP(1 + hidden_size, hidden_size, mlp_size, num_layers, tanh=True)
        self._drift_vol = MLP(1 + hidden_size, hidden_size, mlp_size, num_layers, tanh=True)
        self._diffusion_stock = MLP(1 + hidden_size, hidden_size * noise_size, mlp_size, num_layers, tanh=True)
        self._diffusion_vol = MLP(1 + hidden_size, hidden_size * noise_size, mlp_size, num_layers, tanh=True)


    def f_and_g(self, t, x, y):
        # t has shape ()
        # x has shape (batch_size, hidden_size)
        # y has shape (batch_size, hidden_size)
        t = t.expand(x.size(0), 1)
        tx = torch.cat([t, x], dim=1)
        ty = torch.cat([t, y], dim=1)
        return self._drift_stock(tx), self._drift_vol(ty), self._diffusion_stock(tx).view(x.size(0), self._hidden_size, self._noise_size), self._diffusion_vol(tx).view(y.size(0), self._hidden_size, self._noise_size)
...
class Generator(torch.nn.Module):
    def __init__(self, data_size, initial_noise_size, noise_size, hidden_size, mlp_size, num_layers):
        ...
        self._initial_stock = MLP(initial_noise_size, hidden_size, mlp_size, num_layers, tanh=False)
        self._initial_vol = MLP(initial_noise_size, hidden_size, mlp_size, num_layers, tanh=False)
    ...
    def forward(self, ts, batch_size):
        ...
        x0 = self._initial_stock(init_noise)
        y0 = self._initial_vol(init_noise)
        z0 = torch.cat([x0, y0], dim=2)
        ###################
        # We use the reversible Heun method to get accurate gradients whilst using the adjoint method.
        ###################


        xs = torchsde.sdeint_adjoint(self._func, z0, ts, method='reversible_heun', dt=1.0,
