...
###################
def get_data_volatility(batch_size, device):
    dataset_size = 8192
    t_size = 64
    ...
    class OrnsteinUhlenbeckSDE(torch.nn.Module):
        ...
        def __init__(self, kappa, theta, xi):
            super().__init__()
        ...
        def g(self, t, y):
            # self.sigma*y
            return self.xi*np.sqrt(np.maximum(y, 0))


                #OrnsteinUhlenbeckSDE(mu=0.00, theta=-0.1, sigma=0.4).to(device)
    ou_sde = OrnsteinUhlenbeckSDE(kappa=0.01, theta=-0.1, xi=0.4).to(device)
...
def main(
    ...
    # Models
    generator_vol = Generator(data_size_vol, initial_noise_size, noise_size, hidden_size, mlp_size, num_layers).to(device)
    discriminator_vol = Discriminator(data_size_vol, hidden_size, mlp_size, num_layers).to(device)
    ...
    averaged_generator_vol = swa_utils.AveragedModel(generator_vol)
    averaged_discriminator_vol = swa_utils.AveragedModel(discriminator_vol)
    ...
    with torch.no_grad():
        for param in generator._initial.parameters() and generator_vol._initial.parameters():
            param *= init_mult1
        for param in generator._func.parameters() and generator_vol._func.parameters():
            param *= init_mult2
    ...
    generator_vol_optimiser = torch.optim.Adadelta(generator_vol.parameters(), lr=generator_lr, weight_decay=weight_decay)
    discriminator_vol_optimiser = torch.optim.Adadelta(discriminator_vol.parameters(), lr=discriminator_lr,
                                                   weight_decay=weight_decay)
    # Train both generator and discriminator.
    trange = tqdm.tqdm(range(steps))
    for step in trange:
        real_samples, = next(infinite_train_dataloader)
        real_samples_vol, = next(infinite_train_dataloader_vol)

        generated_samples = generator(ts, batch_size)
        generated_samples_vol = generator_vol(ts_vol, batch_size)

         
        generated_score = discriminator(generated_samples)
        generated_score_vol = discriminator_vol(generated_samples_vol)

        real_score = discriminator(real_samples)
        real_score_vol = discriminator_vol(real_samples_vol)
        loss_vol = generated_score_vol - real_score_vol
        loss = generated_score - real_score

        loss_vol.backward()
        loss.backward()

        for param in generator.parameters() and generator_vol.parameters():
            param.grad *= -1
        
        generator_vol_optimiser.step()
        generator_optimiser.step()
        discriminator_vol_optimiser.step()
        discriminator_optimiser.step()

        generator_vol_optimiser.zero_grad()
        generator_optimiser.zero_grad()
        discriminator_vol_optimiser.zero_grad()
        ...
        ###################
        with torch.no_grad():
            for module in discriminator.modules() or discriminator_vol.modules():
        ...
        if step > swa_step_start:
            ...
            averaged_generator_vol.update_parameters(generator_vol)
            averaged_discriminator_vol.update_parameters(discriminator_vol)

        if (step % steps_per_print) == 0 or step == steps - 1:
            total_unaveraged_loss = evaluate_loss(ts, batch_size, train_dataloader, generator, discriminator)
            total_unaveraged_loss_vol = evaluate_loss(ts_vol, batch_size, train_dataloader_vol, generator_vol, discriminator_vol)
            if step > swa_step_start:
                total_averaged_loss = evaluate_loss(ts, batch_size, train_dataloader, averaged_generator.module,
                                                    averaged_discriminator.module)
                total_unaveraged_loss_vol = evaluate_loss(ts_vol, batch_size, train_dataloader_vol, averaged_generator_vol.module, averaged_discriminator_vol.module)
    ...
    generator_vol.load_state_dict(averaged_generator_vol.module.state_dict())
    discriminator_vol.load_state_dict(averaged_discriminator_vol.module.state_dict())

    generator.load_state_dict(averaged_generator.module.state_dict())
    discriminator.load_state_dict(averaged_discriminator.module.state_dict())

    _, _, test_dataloader = get_data(batch_size=batch_size, device=device)
    _, _, test_dataloader_vol = get_data_volatility(batch_size=batch_size, device=device)

    plot(ts, generator, test_dataloader, num_plot_samples, plot_locs)
    plot(ts_vol, generator_vol, test_dataloader_vol, num_plot_samples, plot_locs)
